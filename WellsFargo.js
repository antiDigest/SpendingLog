function processWhenAccountIsZelle_(message, sentences, row) {
  row["merchant"] = ""
...
function processWhenAccountIsBilt_(message, sentences, row) {
  row["merchant"] = ""
  for (var i=0; i<sentences.length; i++) {
    if (sentences[i].indexOf("Merchant detail") > -1) {
      row["merchant"] = sentences[i+1];
    } else if (sentences[i].indexOf("Amount") > -1) {
      amount = sentences[i+1].split("$")[1];
      if (amount.indexOf(",") > -1) {
        splitAmount = amount.split(",");
        amount = parseFloat(splitAmount[0]) * 1000 + parseFloat(splitAmount[1])
      }
      row["amount"] = amount
    } else if (sentences[i].indexOf("Credit card") > -1) {
      row["last4"] = sentences[i+1].split("...")[1]; 
    } 
